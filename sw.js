...
async function pagina(event) {
    ...
    try {
        ...
        let res = preload || await fetch(req);
        /* Se il server ha risposto con un redirect (es. /pagina.html -> /pagina),
           la risposta porta il marchio "redirected" e il browser si rifiuta di
           usarla per una navigazione: la si riconfeziona identica ma pulita. */
